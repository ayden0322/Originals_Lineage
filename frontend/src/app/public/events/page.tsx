...
export default function EventsPage() {
  ...
  const bannerUrl = settings?.eventsBannerUrl || '';
  const categorySlug = settings?.eventsCategorySlug || '';
  const pageTitle = settings?.eventsPageTitle || '活動';
  const pageSubtitle = settings?.eventsPageSubtitle || '';
  const layout = settings?.eventsLayout || 'magazine';
  const pageSize = settings?.eventsPerPage || 12;
  const showCover = settings?.eventsShowCover !== false;
  const showViewCount = settings?.eventsShowViewCount !== false;
  const showSearch = settings?.eventsShowSearch === true;
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 60, color: 'rgba(255,255,255,0.4)' }}>
            {!categorySlug ? '尚未設定活動分類，請至後台「活動頁面管理」綁定分類' : '暫無活動'}
          </div>
        ) : (
          <>
            {/* ═══ Magazine Layout ═══ */}
