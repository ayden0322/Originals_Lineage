...
export default function EventsSettingsPage() {
  ...
  const [eventsLayout, setEventsLayout] = useState<'magazine' | 'timeline' | 'masonry'>('magazine');
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      form.setFieldsValue({
        eventsPageTitle: settings.eventsPageTitle || '活動',
        eventsPageSubtitle: settings.eventsPageSubtitle || '',
        eventsCategorySlug: settings.eventsCategorySlug || undefined,
        eventsPerPage: settings.eventsPerPage || 12,
        eventsDefaultSort: settings.eventsDefaultSort || 'newest',
        eventsShowCover: settings.eventsShowCover !== false,
        eventsShowViewCount: settings.eventsShowViewCount !== false,
        eventsShowSearch: settings.eventsShowSearch === true,
      });
      setEventsLayout((settings.eventsLayout as 'magazine' | 'timeline' | 'masonry') || 'magazine');
      setBannerUrl(settings.eventsBannerUrl || '');
      setCategories(cats);
    } catch {
  ...
  const handleSave = async () => {
    try {
      ...
      await updateSiteSettings({
        ...values,
        eventsLayout,
        eventsBannerUrl: bannerUrl,
      });
      message.success('活動頁面設定已儲存');
    } catch {
      message.error('儲存失敗');
    } finally {
      setSubmitting(false);
  ...
  return (
    <div>
      <Title level={3}>活動頁面管理</Title>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        {/* ── Layout Selection ── */}
        <Card title="版面配置" loading={loading}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {LAYOUT_OPTIONS.map((opt) => {
              const selected = eventsLayout === opt.key;
              return (
                <div
                  key={opt.key}
                  onClick={() => setEventsLayout(opt.key)}
                  style={{
        ...
        <Card title="Banner 與分類設定" loading={loading}>
          <Form form={form} layout="vertical" style={{ maxWidth: 600 }}>
            <Form.Item name="eventsPageTitle" label="頁面標題">
              <Input placeholder="活動" />
            </Form.Item>
            <Form.Item name="eventsPageSubtitle" label="頁面副標題">
              <Input placeholder="參與最新活動，贏取限定獎勵" />
            </Form.Item>
            <Form.Item
              name="eventsCategorySlug"
              label="綁定文章分類"
            ...
            <Form.Item label="Banner 圖片上傳">
              <ImageUpload value={bannerUrl} onChange={setBannerUrl} folder="events" />
            </Form.Item>
          </Form>
        </Card>
        ...
        <Card title="顯示設定" loading={loading}>
          <Form form={form} layout="vertical" style={{ maxWidth: 600 }}>
            <Form.Item name="eventsPerPage" label="每頁顯示數量">
              <InputNumber min={1} max={100} style={{ width: 200 }} />
            </Form.Item>
            <Form.Item name="eventsDefaultSort" label="預設排序">
              <Select
                style={{ width: 200 }}
                options={[
            ...
            <Form.Item name="eventsShowCover" label="顯示封面圖" valuePropName="checked">
              <Switch />
            </Form.Item>
            <Form.Item name="eventsShowViewCount" label="顯示瀏覽數" valuePropName="checked">
              <Switch />
            </Form.Item>
            <Form.Item name="eventsShowSearch" label="顯示搜尋欄" valuePropName="checked">
              <Switch />
            </Form.Item>
          </Form>
        </Card>
